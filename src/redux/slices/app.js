...
export function FetchAllUsers() {
  return async (dispatch, getState) => {
    try {
      ...
      dispatch(
        slice.actions.updateAllUsers({
          users: response.data.data.map((user) => ({
            ...user,
            image: getMediaUrl(user.avatar),
            online: user.status === "Online",
          })),
        })
      );
    ...
    try {
      ...
      dispatch(
        slice.actions.updateFriends({
          friends: response.data.data.map((user) => ({
            ...user,
            img: getMediaUrl(user.avatar),
            online: user.status === "Online",
          })),
    ...
    try {
      ...
      dispatch(
        slice.actions.fetchUser({
          user: {
            ...response.data.data,
            avatarUrl: getMediaUrl(response.data.data.avatar),
          },
        })
      );
    } catch (error) {
  ...
  return async (dispatch, getState) => {
    try {
      ...
      if (avatar instanceof File) {
        ...
        await fetch(signatureResponse.data.data.signedUrl, {
          method: "PUT",
          body: avatar,
          headers: {
        ...
        avatar = signatureResponse.data.data.key;
      }
